...
def main() -> int:
    ...
    saves = Path(os.path.expanduser("~")) / "Documents" / "My Games" / "Fable" / "Saves" / "0atlas"
    if not saves.is_dir():
        print(f"missing profile folder {saves}: copy an Oakvale autosave profile there first", file=sys.stderr)
        return 2
    ...
    probe.write_text(PROBE_LUA % {"map": a.map, "points": ", ".join("{%g, %g}" % p for p in world_pts),
                                  "teleport": ("{%g, %g}" % (mx + cx, my + cy)) if a.teleport else "nil"}, encoding="utf-8")
    ...
    try:
        ...
        result["ok"] = done and compared == len(pts) and not result["mismatches"]
    finally:

...
void applyTheme() {
    ...
    s.ItemInnerSpacing = ImVec2(6, 6); s.IndentSpacing = 14; s.ScrollbarSize = 10; s.GrabMinSize = 10;
    ImVec4* c = s.Colors;
...
bool toggle(const char* label, bool* value) {
    ...
    const float rowH = std::max(h, textSize.y) + 6;
    ImGui::InvisibleButton("##t", ImVec2(std::max(ImGui::GetContentRegionAvail().x, w + 8 + textSize.x), rowH));
    ...
    const float kx = *value ? p.x + w - h * 0.5f - 2 : p.x + h * 0.5f + 2;
    dl->AddCircleFilled(ImVec2(kx, y + h * 0.5f), h * 0.5f - 3, IM_COL32(255, 255, 255, 255));
    dl->AddText(ImVec2(p.x + w + 10, p.y + (rowH - textSize.y) * 0.5f), col(*value ? Text : Muted), label);
    ImGui::PopID();
    return clicked;
}
...
bool segmented(const char* id, int& value, std::initializer_list<const char*> options, float width) {
    ...
    const float h = 30.0f;
    ImDrawList* dl = ImGui::GetWindowDrawList();
    dl->AddRectFilled(p, ImVec2(p.x + width, p.y + h), col(Bg0), 8.0f);
    bool changed = false;
    const float segW = (width - 4) / n;
    int i = 0;
    for (const char* opt : options) {
        const ImVec2 a(p.x + 2 + segW * i, p.y + 2), b(a.x + segW, p.y + h - 2);
        ImGui::SetCursorScreenPos(a);
        ImGui::InvisibleButton(opt, ImVec2(segW, h - 4));
        const bool hovered = ImGui::IsItemHovered();
        if (ImGui::IsItemClicked() && value != i) { value = i; changed = true; }
        if (value == i) dl->AddRectFilled(a, b, col(Accent), 6.0f);
        else if (hovered) dl->AddRectFilled(a, b, col(Bg3), 6.0f);
        const ImVec2 ts = ImGui::CalcTextSize(opt);
        dl->AddText(ImVec2(a.x + (segW - ts.x) * 0.5f, a.y + (h - 4 - ts.y) * 0.5f),

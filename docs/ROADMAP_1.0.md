...
D3D11 HRESULTs in `--auto` mode and the workflow prints the run log; run 2 showed WARP works and only the screenshot's missing `build/ui/` folder failed; `6d32101` creates it -- **CI GREEN** from run 3).
...
|-------|------------------------|------------------|------|
| **Custom static meshes** (props, buildings) | Compiled-mesh grammar round-trips; `compose_mesh` builds type-1 (static) meshes from arbitrary geometry; `big_write.rebuild(adds=)` appends to graphics.big (`FableTLC docs/formats/MESH.md`, `tools/blender_addon/io_scene_fable/mesh_rw.py`). EgoCore `MeshCompiler.h`/`GltfMeshImporter.h` is the C++ oracle. Atlas already *reads* meshes (`thingsexport`, vendored `meshpreview`). | **CLI DONE 2026-09-20:** `forge mesh-import <model.glb\|.gltf\|.obj> <NAME> [--texture png \| --texture-id n] [--donor OBJECT_...]` -- `forge::meshcompose::composeStatic` (the `compose_mesh` static grammar in C++: one CStaticBlock per primitive, float layout 0x14/20, chunked LZO, the sentinel material, EgoCore's Info blob) + loaders for glTF 2.0 (glb / gltf, node transforms, any component type) and OBJ (usemtl groups) in `src/meshimport` + the append to `MBANK_ALLMESHES` (next id) + `<NAME>_DIFFUSE` into textures.big + an `OBJECT_<NAME>` def (donor copy, `Graphic.modelId` repointed, MeshHeight/Radius from the bounds). Every composed payload is decoded back through `meshpreview` before it is written; `forge-tools mesh-info` inspects an entry. Unit test (compose -> decode) + `tools/test_meshimport.py` (cube .obj and .glb into scratch banks). The editor card *Import model* (Objects sub-tab, off the UI thread, reloads the def/texture context and the thumbnail bank; `tests/ui/meshimport.txt`) DONE the same day. The physics hull DONE the same day (`composePhysics`: EgoCore's `3DMF` type-3 entry from the model's triangles, `PhysicsIndex` wired; retail's `MESH_X[PHYSICS]` pairing confirmed with `mesh-info`); mesh space is centimetres (retail barrel z 0..132), the importer scales metres x100. The hull carries retail's chunk set (TRIS, SMTH, VERT, UNIV, MTLE; formats decoded from `MESH_OBJECT_BARREL[PHYSICS]` with `mesh-info --raw`) with the triangles in the engine's winding (straight order = inside-out hull: the hero walked in and was held at the centre -- three runs, positions logged). **IN-GAME VERIFIED 2026-09-20 evening:** the cube renders lit with its texture where placed, the retail barrel control is solid, cube 4 (flipped winding) stops the hero (user-confirmed). Left on the install: 4 probe cubes + a barrel in StartOakValeWest and their mesh/def/texture entries (`release_probes.py --stage mesh_undo` puts the files back). | L |

...
| `mods_tab 0\|1`, `mod_add <path> [name]`, `mod_remove <name\|index>`, `mod_move <name\|index> <to>`, `mod_enable <name\|index> 0\|1`, `mods_deploy`, `mods_undeploy`, `mods_conflicts`, `wait_mods` | the Mods tab: the save root's `forge_mods.json` load order; deploy/undeploy/conflicts run `forge-tools.exe` and stream into the log (state `mods_mode`, `mods_count`; widgets `input_mod_path`, `input_mod_name`, `btn_mod_add`, `btn_mods_conflicts`, `btn_mods_deploy`, `btn_mods_undeploy`) |
